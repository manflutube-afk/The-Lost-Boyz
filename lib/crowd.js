...
export const LIMITS = {
  ...
  words: 1200,
  clip: 300,
  /* Roughly 1.1MB of image once the base64 is unwrapped. A photo resized to
     1600px lands nearer 300KB, so this is loose enough for an odd one and
     tight enough that nobody is filling the store with a single request. */
...
/*
 * A link to a clip, if it is one of the places clips actually live. An open
 * text field taking any URL on a form anybody can submit is a spam magnet, and
 * an approved link is one the band have seen but not necessarily followed.
 */
export function cleanClip(value) {
  const raw = clean(value, LIMITS.clip);
  if (!raw) { return ''; }
  ...
  const allowed = /^(youtube\.com|youtu\.be|m\.youtube\.com|facebook\.com|fb\.watch|instagram\.com|tiktok\.com|vimeo\.com)$/;
  if (!allowed.test(host)) { return ''; }

  url.protocol = 'https:';
  return url.toString().slice(0, LIMITS.clip);
}
...
export function tidy(input) {
  return {
    ...
    clip: cleanClip(input.clip),
    photo: false,          // set by the endpoint once the bytes are checked
    photoType: '',
    state: 'pending',
  };
}
